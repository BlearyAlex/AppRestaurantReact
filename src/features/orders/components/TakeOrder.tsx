...
import CurrentConsumptionModal from "@/features/orders/components/CurrentConsumptionModal.tsx";
import { Button } from "@/components/ui/button.tsx";
import { OrderType } from "@/enums/orderEnum.ts";
import type { CreateOrderDto } from "@/features/orders/types/order";
...
function TakeOrder({ orderType }: { orderType: OrderType }) {
    ...
    const filteredProducts = useMemo(() => {
        let result = data.filter((p) => p.isActive)
        ...
        return result
    }, [selectedCategory, data, searchQuery])
